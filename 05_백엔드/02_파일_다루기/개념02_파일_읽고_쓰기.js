...
const lines_1 = text.split("\n").filter((line) => line !== "");
console.log("마지막줄 : ", lines.at(-1));
// 마지막줄 :  작성일: 2026-03-05
...
//                    그다음 탐색기에서 열어 확인하세요.
const 메모경로 = path.join(__dirname, "메모.txt");
fs.writeFileSync(메모경로, "김명재", "utf-8");
console.log(fs.readFileSync(메모경로, "utf-8"));
// 김명재
...
fs.writeFileSync(testPath, "곧 지워질 파일", "utf-8");
console.log(fs.existsSync(testPath)); // true
fs.unlinkSync(testPath);
console.log(fs.existsSync(testPath)); // false
