...
console.log("------");
const u = new URL("http://localhost:3000/search?q=작업표준서&limit=10");
console.log("q: ", u.searchParams.get("q"));
console.log("limit", u.searchParams.get("limit"));
console.log("limit typeof", typeof u.searchParams.get("limit"));
console.log("------");
...
const locH = new URL("http://localhost:4000/api/documents");
console.log("port", locH.port); // port 4000
...
const 직접해보기4 = new URL("http://localhost:3000/api/lines/A/machines/3");
console.log(
  "직접해보기4: ",
  직접해보기4.pathname.split("/").filter((s) => s !== ""),
);
// 직접해보기4:  [ 'api', 'lines', 'A', 'machines', '3' ]
